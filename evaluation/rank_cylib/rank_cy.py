...
# Main interface
def evaluate_cy(indices: cython.long[:, :], q_pids: cython.long[:], g_pids: cython.long[:], q_camids: cython.long[:], g_camids: cython.long[:], max_rank: cython.long, sysu: cython.bint, new_cmc: cython.bint):
    ...
    if new_cmc:
        new_raw_cmc: cython.float[:] = np.zeros(num_g, dtype=np.float32)
        pred_label: cython.long[:, :] = np.zeros((num_q, num_g), dtype=np.int64)
        for q_idx in range(num_q):
            for g_idx in range(num_g):
                pred_label[q_idx, g_idx] = g_pids[indices[q_idx, g_idx]]
        new_pred_label: cython.long[:] = np.zeros(num_g, dtype=np.int64)
        
        _i: cython.long
        index: cython.long
        
        new_avg_cmc: cython.float[:] = np.zeros(max_rank, dtype=np.float32)
        new_matches: cython.float[:] = np.zeros(num_g, dtype=np.float32)
    ...
    for q_idx in range(num_q):
        ...
        for g_idx in range(num_g):
            if sysu:
                if g_camids[order[g_idx]] != 2 or q_camid != 3:
                    # in sysu，camid 2 and 3 are the same location
                    raw_cmc[num_g_real] = matches[g_idx]
                    if new_cmc:
                        new_pred_label[num_g_real] = pred_label[q_idx, g_idx]
                    num_g_real += 1
                    # this condition is true if query appear in gallery
                    if not meet_condition and matches[g_idx] > 1e-31:
                        meet_condition = True
            else:
        ...
        if new_cmc:
            ...
            new_index = sorted(np.unique(np.array(new_pred_label)[:num_g_real], return_index=True)[1])
            _i = 0
            for index in new_index:
                new_matches[_i] = (new_pred_label[index] == q_pid) * 1.0
                _i += 1
            
            # print(q_idx, _i, new_matches[0], new_matches[1], new_matches[2], new_matches[3], new_matches[4])
            function_cumsum(new_matches, new_raw_cmc, _i)
            for rank_idx in range(max_rank):
                new_avg_cmc[rank_idx] += new_raw_cmc[rank_idx]
    ...
    if new_cmc:
        for rank_idx in range(max_rank):
            new_avg_cmc[rank_idx] /= num_valid_q

    
    if new_cmc:
        return np.asarray(new_avg_cmc).astype(np.float32), np.asarray(all_AP[:valid_index]), np.asarray(all_INP[:valid_index])
    else:
