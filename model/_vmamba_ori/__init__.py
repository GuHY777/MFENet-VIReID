...
class mamba_init:
    ...
    def A_log_init(d_state, d_inner, copies=-1, device=None, merge=True):
        ...
        if copies > 0:
            ...
            if merge:
                A_log = A_log.flatten(0, 1) # (copies * inner, state)
        A_log = nn.Parameter(A_log)
        A_log._no_weight_decay = True
        return A_log # (copies, inner, state) or (inner, state)
    ...
    def D_init(d_inner, copies=-1, device=None, merge=True):
        ...
        if copies > 0:
            ...
            if merge:
                D = D.flatten(0, 1) # (copies * inner)
        D = nn.Parameter(D)  # Keep in fp32
        D._no_weight_decay = True
        return D # (copies, inner) or (inner)
    ...
    def init_dt_A_D(cls, d_state, dt_rank, d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, k_group=4):
        ...
        dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in dt_projs], dim=0)) # (K, D, R)
        dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in dt_projs], dim=0)) # (K, D)
        del dt_projs
...
class SS2Dv0:
    def __initv0__(
        ...
        **kwargs,
        ...
        super().__init__()
        d_inner = int(ssm_ratio * d_model) # inner dim of SSM
        dt_rank = math.ceil(d_model / 16) if dt_rank == "auto" else dt_rank # intermediary rank of dt proj
        ...
        # in proj ============================
        self.in_proj = nn.Linear(d_model, d_inner * 2, bias=bias) # (b, h, w, d_model) -> (b, h, w, d_inner*2)              
        self.act: nn.Module = act_layer()
        self.conv2d = nn.Conv2d( # Conv2d not Conv1d, so it's called SS2D
            in_channels=d_inner,
            out_channels=d_inner,
            groups=d_inner, #this is depthwise conv
            bias=conv_bias,
            kernel_size=d_conv,
            padding=(d_conv - 1) // 2,
            **factory_kwargs,
        ...
        # x proj ============================
        self.x_proj = [
            nn.Linear(d_inner, (dt_rank + d_state * 2), bias=False) # intermediate dt, B, C
            for _ in range(k_group) # four directions
        ]
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, (dt_rank + d_state * 2), d_inner)
        del self.x_proj
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            ...
            v05=partial(self.forward_corev2, force_fp32=True, no_einsum=True),  # selective_scan_backend="oflex", scan_mode="cross2d"
            # ===============================
