...
class IMA(nn.Module):
    ...
    def forward(self, x):
        ...
        x_amps = torch.stack(x_amps, dim=2) # B,C,N,H,W
        ws = torch.stack(ws, dim=2).unsqueeze(3) # B,C,N,1
        ws = self.ws(ws).unsqueeze(4) # B,1,N,1,1
        x_amp = (x_amps * ws).sum(2) # B,C,H,W
        x_ = self.ifft(torch.polar(x_amp, x_pha))
        
        # if self.i % 600 == 0:
        #     logger.info(f"all:\n {stats.describe(ws.cpu().detach().flatten().numpy())}")
        #     # logger.info(f"C-1:\n {stats.describe(ws[:,0].cpu().detach().flatten().numpy())}")
        #     # logger.info(f"C-100:\n {stats.describe(ws[:,99].cpu().detach().flatten().numpy())}")
        #     if x.size(0) > 1:
        #         logger.info(f"N:\n {ws[0].cpu().detach().flatten().numpy()}\n {ws[1].cpu().detach().flatten().numpy()}\n {ws[32].cpu().detach().flatten().numpy()}")
        # self.i += 1
...
class NormLinear(nn.Linear):
    ...
    def forward(self, input):
        ...
        return F.linear(norm_input, norm_weight, self.bias)

@MODEL_REGISTRY.register()
class BaseLine(nn.Module):
    def __init__(self, backbone_name='resnet50', num_classes=751, iid_vi=False, *args, **kwargs):
        super().__init__()
        ...
        layers = nn.Sequential(
            ...
            LHM([1024, *(img_size[0]//16, img_size[1]//16)], 0.2),
            bb.layer3[5],
            IMA([1024, *(img_size[0]//16, img_size[1]//16)], 5),
        ...
        nn.init.normal_(self.cls.weight.data, 0, 0.001)
    ...
    def forward(self, x, cam_ids, *args, **kwargs):
        ...
        f0 = self.share_module(x)
            
        pooled_f = self.pool(f0).flatten(1)
        f = self.bn_neck(pooled_f)
        if not self.training:
            return (f, torch.cat([F.normalize(pooled_f, dim=1), F.normalize(f, dim=1)], dim=1))
            
        return [self.cls(f),], [pooled_f,], [pooled_f,], [F.normalize(f, dim=1),]
