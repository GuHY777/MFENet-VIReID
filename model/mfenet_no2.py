...
class MFENet_no2(nn.Module):
    def __init__(self, backbone_name='resnet50', num_classes=751, B=7, N=3, use_HiLo_FM=True, use_FADE=True, *args, **kwargs):
        super().__init__()
        
        self.N = N
        self.use_HiLo_FM = use_HiLo_FM
        self.use_FADE = use_FADE
        if not use_FADE:
            self.N = N = 1 # force to use only one scale
        ...
        self.share_module = nn.Sequential(
            ...
            bb.layer2[1],
            bb.layer2[2],
            HiLo_FM([512, *(img_size[0]//8, img_size[1]//8)], 0.2) if use_HiLo_FM else nn.Identity(),
            bb.layer2[3],
            HiLo_FM([512, *(img_size[0]//8, img_size[1]//8)], 0.2) if use_HiLo_FM else nn.Identity(),
            ...
            bb.layer3[2],
            bb.layer3[3],
            HiLo_FM([1024, *(img_size[0]//16, img_size[1]//16)], 0.2) if use_HiLo_FM else nn.Identity(),
            bb.layer3[4],
            HiLo_FM([1024, *(img_size[0]//16, img_size[1]//16)], 0.2) if use_HiLo_FM else nn.Identity(),
            bb.layer3[5],
            FADE([1024, *(img_size[0]//16, img_size[1]//16)], B, N) if use_FADE else nn.Identity(),
        )
        
        self.layer4s = nn.ModuleList([
            deepcopy(bb.layer4) for _ in range(N)
        ])
        
        self.pools = nn.ModuleList([
            GeM(p=3.0) for _ in range(N)
        ])
        ...
        self.bn_necks = nn.ModuleList([
            deepcopy(bn_neck) for _ in range(N)
        ])

        self.clss = nn.ModuleList([
            nn.Linear(2048, num_classes, bias=False) for _ in range(N) 
        ])
        
    def forward(self, x, cam_ids, *args, **kwargs):
        f0 = self.share_module(x) if self.use_FADE else [self.share_module(x)]
        f1 = [self.layer4s[i](f0[i]) for i in range(self.N)]
        pf = [self.pools[i](f1[i]).flatten(1) for i in range(self.N)]
        f = [self.bn_necks[i](pf[i]) for i in range(self.N)]

        if not self.training:
            return torch.cat([*[NN(fi) for fi in f], *[NN(pfi) for pfi in pf]], dim=1)
        
        logits = [self.clss[i](f[i]) for i in range(self.N)]
