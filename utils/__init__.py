...
def setup(args, fileout=True, determenistic=True, benchmark=False):
    ...
    if old_args_dict['config']:
        ...
        # remain the exp and gpus in new args
        yaml_args.pop('exp')
        yaml_args.pop('gpus')     
        args.__dict__.update(yaml_args)
        args.config = '' # do not save the config in the new args
    ...
    with open(os.path.join(savedir, f"{args.model}-{args.dataset}.yaml"), 'w') as f:
